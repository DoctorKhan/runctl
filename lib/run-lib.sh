...
run_global_list_live_listeners() {
  if ! command -v lsof >/dev/null 2>&1; then
    printf 'live listeners (cwd-derived)\n'
    printf '  %s\n' "(lsof missing)"
    return 0
  fi

  local projects_root="${RUNCTL_PROJECTS_ROOT:-$HOME/Documents/Projects}"
  local tmp
  tmp="$(mktemp "${TMPDIR:-/tmp}/runlib-live.XXXXXX")"
  ...
  lsof -nP -iTCP -sTCP:LISTEN 2>/dev/null \
    ...
    | while read -r pid; do
        ...
        local cwd prog ports
        cwd="$(run_pid_cwd "$pid" || true)"
        [[ -n "$cwd" ]] || continue
        case "$cwd" in
        ...
        case "$cwd" in
          *"/runctl-kit" | *"/runctl-kit/"* | *"/runctl-kit"*) continue ;;
        esac
        ports="$(
        ...
        [[ -n "$ports" ]] || continue
        if ! printf '%s' "$ports" | tr ',' '\n' | awk '$1>=3000 && $1<=3999 {found=1} END{exit(found?0:1)}'; then
          continue
        fi
        prog="$(run_pid_program_name "$pid" || true)"
        [[ -n "$prog" ]] || prog="(unknown)"
        printf '%s\t%s\t%s\t%s\n' "$pid" "$ports" "$prog" "$cwd" >>"$tmp"
      done

  printf 'live listeners (cwd-derived)\n'
  printf '%-8s %-12s %-20s %s\n' "PID" "PORTS" "PROGRAM" "CWD"
  printf '%-8s %-12s %-20s %s\n' "--------" "------------" "--------------------" "----------------------------------------------"
  if [[ -s "$tmp" ]]; then
    sort -n -k1,1 "$tmp" | while IFS=$'\t' read -r pid ports prog cwd; do
      printf '%-8s %-12s %-20s %s\n' "$pid" "$ports" "$prog" "$cwd"
    done
  else
    printf '%s\n' "(no live listeners)"
...
run_project_refresh_from_listener() {
  local project_root="$1"
  local pid="$2"
  local port="$3"
  local service="${4:-web}"
  [[ -n "$project_root" && -n "$pid" && -n "$port" ]] || return 1

  local local_state="$project_root/.run"
  mkdir -p "$local_state/pids" "$local_state/logs"
  [[ -f "$local_state/claimed-ports" ]] || : >"$local_state/claimed-ports"

  cat >"$local_state/ports.env" <<EOF
# Generated by runctl (live listener reconcile)
PORT=$port
RUN_DEV_PORT=$port
RUN_DEV_SERVICE=$service
HOST=127.0.0.1
EOF
  printf '%s\n' "$pid" >"$local_state/pids/${service}.pid"
  if ! grep -qx "$port" "$local_state/claimed-ports" 2>/dev/null; then
    printf '%s\n' "$port" >>"$local_state/claimed-ports"
  fi

  local slug global_state
  slug="$(printf '%s' "$project_root" | shasum -a 256 | awk '{print substr($1,1,16)}')"
  global_state="${RUN_GLOBAL_STATE:-$HOME/.run}"
  mkdir -p "$global_state/ports" "$global_state/projects/$slug"
  {
  ...
  } >"$global_state/ports/$port"
}

run_global_reconcile_projects_from_live_listeners() {
  if ! command -v lsof >/dev/null 2>&1; then
    return 0
  fi
  local projects_root="${RUNCTL_PROJECTS_ROOT:-$HOME/Documents/Projects}"
  local pid cwd port service ports
  while IFS= read -r pid; do
    ...
    while IFS= read -r port; do
      [[ "$port" =~ ^[0-9]+$ ]] || continue
      [[ "$port" -ge 3000 && "$port" -le 3999 ]] || continue
      service="web"
      if [[ -f "$cwd/.run/ports.env" ]]; then
        service="$(sed -n 's/^RUN_DEV_SERVICE=//p' "$cwd/.run/ports.env" | head -n1)"
        [[ -n "$service" ]] || service="web"
      fi
      run_project_refresh_from_listener "$cwd" "$pid" "$port" "$service"
    done <<<"$ports"
  done < <(
    lsof -nP -iTCP -sTCP:LISTEN 2>/dev/null \
      | awk 'NR>1 {print $2}' \
